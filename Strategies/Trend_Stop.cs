...
namespace AgenaTrader.UserCode
{
    ...
    public class Trend_Stop : UserStrategy
    {
        ...
        private DataSeries dStopp;              // Datensereie für den Stopp
            ...
            if (execution.Order != null && execution.Order.OrderState == OrderState.Filled)
            {
                if (oTStop != null && execution.Name == oTStop.Name)
                {
                    ...
                    if (TotalSchutz)
                    {
                        ...
                        Stopp = Instrument.Round2TickSize((Close[0] - Trade.AvgPrice) / 2 + Trade.AvgPrice);     // Stopp auch in der Serie zurücksetzen
                        dStopp[1] = Stopp;
                        dStopp[0] = Stopp;
