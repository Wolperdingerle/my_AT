...
namespace AgenaTrader.UserCode
{
	...
	[Description("Short ReversalBar with long wick and close of the bar is greater than the open of the bar")]
	public class ReversalShort_GreenBar : UserScan
	{
		protected override void OnInit()
		{
			...
			ArrowColor = Color.FromArgb(255, 51, 204, 51);
		...
		protected override void OnCalculate()
		{
			LastOutput = MultiBars.GetBarsItem(TimeFrame).Close[0] > MultiBars.GetBarsItem(TimeFrame).Open[0]
				&& ((MultiBars.GetBarsItem(TimeFrame).High[0]-MultiBars.GetBarsItem(TimeFrame).Close[0])) > ((MultiBars.GetBarsItem(TimeFrame).Close[0]-MultiBars.GetBarsItem(TimeFrame).Low[0]))
				&& ((MultiBars.GetBarsItem(TimeFrame).Close[0]-MultiBars.GetBarsItem(TimeFrame).Low[0])) < (((MultiBars.GetBarsItem(TimeFrame).High[0]-MultiBars.GetBarsItem(TimeFrame).Low[0])*(0.45+0)));
		}
	}
	...
	{
		public ReversalShort_GreenBar ReversalShort_GreenBar()
		{
			return GetCondition<ReversalShort_GreenBar>();
		}

		public ReversalShort_GreenBar ReversalShort_GreenBar(IDataSeries input)
		{
			return GetCondition<ReversalShort_GreenBar>(input);
		}
	}
