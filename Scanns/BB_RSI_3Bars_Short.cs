...
{
	[UsedCalculationUnit(typeof(global::AgenaTrader.Custom.Bollinger), typeof(global::AgenaTrader.Custom.RSI), typeof(AgenaTrader.UserCode.ReversalShort_GreenBar), typeof(AgenaTrader.UserCode.ReversalShort_RedBar))]
	[ScanTypeAttribute(ScanType.Short)]
	[IsEntryAttribute(true)]
	[IsStopAttribute(false)]
	[IsTargetAttribute(false)]
	[Category("BollingerRSI")]
	[Description("Signal when overbought RSI situtation combined with Prices outside of BollingerBands")]
	public class BB_RSI_3Bars_Short : UserScan
	{
		protected override void OnInit()
		{
			...
			ArrowColor = Color.FromArgb(255, 0, 102, 255);
			ArrowSize = 0.5;
			TextColor = null;
			ScanColor = Color.FromArgb(255, 0, 102, 255);
			BarHighlightColor = Color.FromArgb(255, 0, 102, 255);
		}

		protected override void OnCalculate()
		{
			LastOutput = MultiBars.GetBarsItem(TimeFrame).High[2] >= LeadIndicator.Bollinger(MultiBars.GetBarsItem(TimeFrame).Close, 2, 20).Upper[2]
				&& MultiBars.GetBarsItem(TimeFrame).High[1] >= LeadIndicator.Bollinger(MultiBars.GetBarsItem(TimeFrame).Close, 2, 20).Upper[1]
				&& MultiBars.GetBarsItem(TimeFrame).High[0] >= LeadIndicator.Bollinger(MultiBars.GetBarsItem(TimeFrame).Close, 2, 20).Upper[0]
				&& LeadIndicator.RSI(MultiBars.GetBarsItem(TimeFrame).Close, 14, 3).Default[0] > 70
				&& (Math.Abs(LeadIndicator.GetCondition("ReversalShort_GreenBar")[0] - 1) <= Double.Epsilon
